...
public class JwtTokenService {
    ...
    public Date calculateExpiration(TokenType tokenType) {
        switch (tokenType) {
            case AUTH_TOKEN:
                return new Date(System.currentTimeMillis() + AppConstants.JWT_AUTH_EXPIRATION);
    ...
    public Boolean isTokenValid(String subject, String token, TokenType expectedTokenType) {
        JWTVerifier jwtVerifier = getJwtVerifier();
        return StringUtils.isNotEmpty(subject) && !isTokenExpired(jwtVerifier, token)
